...
export const TesseractProvider = ({ 
  workerCreateOption, children, onLoad, useSchedule = true, workerNumber = 3, 
  loadLanguage = 'eng', initialize = 'eng', workerParameter = {}
}) => {
  const [loaded, setLoaded] = useState(false);

  const handleOnLoaded = useCallback(({ workers, scheduler }) => {
    if(onLoad){
  ...
  const workers = useMemo(() => Promise.all((arr => arr.fill())(new Array(workerNumber)).map(async () => {
  ...
  //   await worker.loadLanguage(loadLanguage);
  //   await worker.initialize(initialize);
  //   workerParameter && await worker.setParameters(workerParameter);
  //   return worker;
  // })).then((workers) => {
  //   if (useSchedule) {
  //     workers.forEach((worker) => { scheduler.addWorker(worker) });
  ...
  // }).then(({ workers, scheduler }) => {
  //   handleOnLoaded({ workers, scheduler });
  // }).catch((error) => {
  //   console.error(error);
  //   console.error(new Date(), 'fail on load workers');
  // })
